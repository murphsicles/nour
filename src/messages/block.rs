...
impl Block {
    ...
    ///
    /// # Errors
    /// `Error::BadData` if double spend (non-coinbase).
    pub fn inputs(&self) -> Result<HashSet<OutPoint>> {
        let mut inputs = HashSet::new();
        for txn in &self.txns {
            if !txn.coinbase() {
                for input in &txn.inputs {
                    if !inputs.insert(input.prev_output.clone()) {
                        return Err(Error::BadData("Input double spent".to_string()));
                    }
                }
            }
        }
    ...
    ///
    /// # Errors
    /// `Error::BadData` if invalid merkle, no/multiple coinbase, tx invalid.
    pub fn validate(
        &self,
        height: i32,
        network: Network,
    ...
    pub fn merkle_root(&self) -> Hash256 {
        ...
        while row.len() > 1 {
            ...
            while n > 0 {
                ...
                row.push_back(bh_sha256d::Hash::hash(&h).into());
            }
        }
        row.pop_front().unwrap()
    }
...
impl Serializable<Block> for Block {
    fn read(reader: &mut dyn Read) -> Result<Block> {
        ...
        let txn_count = var_int::read(reader)? as usize;
        if txn_count > MAX_TXNS_COUNT {
            return Err(Error::BadData(format!("Too many txns: {}", txn_count)));
        }
...
impl AsyncSerializable<Block> for Block {
    async fn read_async(reader: &mut dyn AsyncRead) -> Result<Block> {
        let header = BlockHeader::read_async(reader).await?;
        let txn_count = var_int::read_async(reader).await? as usize;
        if txn_count > MAX_TXNS_COUNT {
            return Err(Error::BadData(format!("Too many txns: {}", txn_count)));
        }
        let mut txns = Vec::with_capacity(txn_count);
        for _ in 0..txn_count {
            txns.push(Tx::read_async(reader).await?);
        }
        Ok(Block { header, txns })
    }
...
mod tests {
    ...
    fn write_read() {
        ...
        assert_eq!(Block::read(&mut Cursor::new(&v)).unwrap(), block);
    }
}
