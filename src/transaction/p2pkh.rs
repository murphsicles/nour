...
pub fn create_lock_script(address: &Hash160) -> Script {
    ...
    script.append_data(&address.0).unwrap();
...
mod tests {
    ...
    fn check_unlock_script_test() {
        ...
        sig71pkh33.append_data(&[0; 71]).unwrap();
        assert!(!check_unlock_script(&sig71pkh33.0));
        sig71pkh33.append_data(&[0; 33]).unwrap();
        assert!(check_unlock_script(&sig71pkh33.0));
        sig71pkh33.append(OP_1);
        assert!(!check_unlock_script(&sig71pkh33.0));

        let mut sig73pkh65 = Script::new();
        sig73pkh65.append_data(&[0; 73]).unwrap();
        sig73pkh65.append_data(&[0; 65]).unwrap();
        assert!(check_unlock_script(&sig73pkh65.0));

        let mut sig72pkh30 = Script::new();
        sig72pkh30.append_data(&[0; 72]).unwrap();
        sig72pkh30.append_data(&[0; 30]).unwrap();
    ...
    fn check_unlock_script_addr_test() {
        let mut s = Script::new();
        s.append_data(&[5; 71]).unwrap();
        s.append_data(&[6; 65]).unwrap();
