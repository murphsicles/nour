...
fn legacy_sighash(
    ...
    let tx_out_list = if base_type == SIGHASH_NONE {
        ...
        if n_input >= tx.outputs.len() {
            return Err(Error::BadArgument("No matching output for SINGLE".to_string()));
        }
        tx.outputs[..=n_input].to_vec()
    } else {
        tx.outputs.clone()
    };
    var_int::write(tx_out_list.len() as u64, &mut s)?;
    for (i, out) in tx_out_list.iter().enumerate() {
        if i == n_input && base_type == SIGHASH_SINGLE {
            let empty = TxOut { satoshis: -1, lock_script: Script(vec![]) };
            empty.write(&mut s)?;
        } else {
            out.write(&mut s)?;
        }
